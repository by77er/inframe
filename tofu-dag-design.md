...
`tofu-dag` is a typed, code-generated frontend for OpenTofu.
...
`tofu-dag` must not implement its own resource-state engine.
...
`tofu-dag` should treat raw state as an OpenTofu implementation concern.
...
Version pinning is required for reproducible generation.
...
tofu-dag graph validate build/prod.graph.json
tofu-dag graph inspect build/prod.graph.json

# lower without executing
tofu-dag render \
  --graph build/prod.graph.json \
  --out .tofu-dag/stacks/prod/main.tofu.json

# lower + OpenTofu
tofu-dag init     --stack prod --graph build/prod.graph.json
tofu-dag validate --stack prod --graph build/prod.graph.json
tofu-dag plan     --stack prod --graph build/prod.graph.json
tofu-dag apply    --stack prod --graph build/prod.graph.json
tofu-dag destroy  --stack prod --graph build/prod.graph.json
tofu-dag show     --stack prod
tofu-dag output   --stack prod
```
...
spago run --main Infra.Prod \
  | tofu-dag plan --stack prod --graph -
```
...
The `tofu-dag` CLI may eventually provide:

```bash
tofu-dag state login
```
...
```text
tofu-dag provider inspect digitalocean/digitalocean@2.100.0
```
