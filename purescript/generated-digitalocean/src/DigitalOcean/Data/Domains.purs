module DigitalOcean.Data.Domains
  ( Args
  , Required
  , Domains
  , DomainsDataSource
  , args
  , read
  , filter
  , id
  , sort
  ) where

import Prelude (bind, pure)

import Data.Argonaut.Core (Json)
import Foreign.Object as Object
import TofuDag.Builder (Infra, addDataSource)
import TofuDag.Core (Expr, Input, DataSource, inputJson, dataSourceAttr)

data DomainsDataSource

type Required =
  {
  }

newtype Args = Args (Object.Object Json)

args :: Required -> Args
args _ = Args (Object.fromFoldable
  [
  ])

filter :: Input (Array ({ all :: Boolean, key :: String, matchBy :: String, values :: Array String })) -> Args -> Args
filter value (Args values) = Args (Object.insert "filter" (inputJson value) values)

id :: Input String -> Args -> Args
id value (Args values) = Args (Object.insert "id" (inputJson value) values)

sort :: Input (Array ({ direction :: String, key :: String })) -> Args -> Args
sort value (Args values) = Args (Object.insert "sort" (inputJson value) values)

type Domains =
  { dataSource :: DataSource DomainsDataSource
  , domains :: Expr (Array ({ name :: String, ttl :: Number, urn :: String }))
  , id :: Expr String
  }

read :: String -> Args -> Infra Domains
read logicalName (Args values) = do
  handle <- addDataSource "digitalocean_domains" logicalName values
  pure
    { dataSource: handle
    , domains: dataSourceAttr handle [ "domains" ]
    , id: dataSourceAttr handle [ "id" ]
    }
